...
def test_get_list_employers():
    db.create_company('Keks test', 'cool_company')
    ...
    db.create_employer(max_id, "Keks", "Petrov", 800400500)# Получение списков работодателей из базы данных и API
...
def test_assertion_data():
    ...
    db.create_employer(max_id, "Keks", "Petrov", "800400500")
    ...
    assert get_api_info["firstName"] == "Keks"
    assert get_api_info["lastName"] == "Petrov"
...
def test_and_nwe_employer():
    ...
    assert response["firstName"]== "Keks"
    assert response["isActive"]== True
    assert response["lastName"] == "Petrov"
    db.delete_employer(employer_id)
   # db.delete(max_id)
